...
// SUPERGLOBALS
// Merupakan variable global milik PHP
...
<body>
    ...
    <ul>
        <?php foreach($movies as $mov) : ?>
            <li><a href="latihan2.php?title=<?=$mov['title']?>&year=<?=$mov['year']?>&genre=<?=$mov['genre']?>&runtime=<?=$mov['runtime']?>&poster=<?=$mov['poster']?>"><?=$mov["title"];?></a></li>
        <?php endforeach; ?>
    </ul>
</body>
</html>